...
- [ ] Phase 5: 品質向上 & 堅牢化 (Engineering Excellence)
    ...
    - [ ] **Observability**: JSON 形式の構造化ロギング導入とメトリクス計測
    - [x] **Engineering Excellence 修業 (completed)**:
        - `NotionClient` / `SnowflakeClient` の適切な例外移行
        - `tenacity` を活用した「賢いリトライ」と、それを支える高度な統合テスト手法の確立
        - `asyncio.sleep` のモックによる爆速なリトライテスト環境の構築

### 📅 次のタスク (Quality & Observability)
- [ ] **Observability**: `structlog` 等を用いた JSON 形式の構造化ロギングへの移行
- [ ] **Snowflake 連携の高度化**: `SnowflakeClient` でのエラーコード判別（権限、オブジェクト不在等）の厳密化
- [ ] **E2E Testing**: モックを使わないサンドボックス環境での最終疎通確認
