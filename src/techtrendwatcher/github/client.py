...
from techtrendwatcher.core.exceptions import GitHubAPIError, GitHubAuthError, GitHubRateLimitError, GitHubValidationError
...
class GithubClient:
    ...
    @retry(
        # 待ち時間を4秒から開始、最大60秒まで
        wait=wait_exponential(multiplier=GITHUB_RETRY_MULTIPLIER, min=GITHUB_RETRY_MIN_WAIT, max=GITHUB_RETRY_MAX_WAIT),
        # 最大リトライ５回
    ...
    async def search_github(self, query: str) -> GithubAPIFull:
        ...
        try:
            ...
            if status_code == 401:
                raise GitHubAuthError("Github認証エラーが発生しました。認証トークンを確認してください", status_code=status_code, original_error=e) from e
            
            if status_code in (403,429):
                raise GitHubRateLimitError("レート制限よ！少し頭を冷やしなさい！", status_code=status_code, original_error=e) from e
            
            if status_code == 422:
                raise GitHubValidationError("クエリがデタラメよ！", status_code=422, original_error=e) from e
            
            raise GitHubAPIError(f"GitHub APIでエラーが発生したわよ: {e}", status_code=status_code, original_error=e) from e
