...
"""
def save_as_parquet(df:pl.DataFrame,query:str):
...
def get_trend_dataframe(current_df:pl.DataFrame,query:str):
   logger = get_logger(__name__)
   ...
   if len(sorted_files) >= 2:
      ...
      logger.warning(f"過去に処理したparquetファイルが見つかりません")
      return current_df.with_columns(
         star_delta = pl.col("stargazers_count")
      )
